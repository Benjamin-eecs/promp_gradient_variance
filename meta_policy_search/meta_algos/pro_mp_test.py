...
class ProMP_test(MAMLAlgo):
    ...
    def __init__(
        ...
        super(ProMP_test, self).__init__(*args, **kwargs)
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name):
            ...
            obs_phs_h, action_phs_h, adv_phs_h, dist_info_old_phs_h, all_phs_dict_h = self._make_input_placeholders('step2')
            self.meta_op_phs_dict.update(all_phs_dict)
            self.meta_op_phs_dict.update(all_phs_dict_h)

            distribution_info_vars, distribution_info_vars_h, current_policy_params = [], [], []
        ...
        for i in range(self.meta_batch_size):
            ...
            distribution_info_vars_h.append(self.policy.distribution_info_sym(obs_phs_h[i], params=None))
        ...
        with tf.variable_scope(self.name):
            ...
            for step_id in range(1, self.num_inner_grad_steps+1):
                ...
                for i in range(self.meta_batch_size):
                    ...
                    surr_loss2 = self._adapt_objective_sym(action_phs_h[i], adv_phs_h[i], dist_info_old_phs_h[i], distribution_info_vars_h[i])
                    kl_loss = 0.5 * tf.reduce_mean(self.policy.distribution.kl_sym(dist_info_old_phs[i], distribution_info_vars[i])) + 0.5 * tf.reduce_mean(self.policy.distribution.kl_sym(dist_info_old_phs_h[i], distribution_info_vars_h[i]))

                    adapted_params_var, gradient_vector = self._adapt_sym_both(surr_loss, surr_loss2, current_policy_params[i])
